...
void SchedulingBS::MuMimoSchedule(int bsID, int subband, arma::uvec scheduluedMS, int link)
{
	...
	if (BS[bsID]->channel->NumAssociatedMS > 0)
	{
		...
		int numMaxLayer = std::min(Sim.scheduling->numMaxLayer, BS[bsID]->channel->NumAssociatedMS); // 拌魂 汗棱档 临捞扁 困秦
		if (numMaxLayer == 0);
		/*
		{
			BS[bsID]->scheduling->scheduledMS.zeros(1);
			BS[bsID]->scheduling->scheduledMS(0) = -1;
		}
		*/
		else
		{
			...
			for (int attachedMsID = 0; attachedMsID < BS[bsID]->channel->NumAssociatedMS; attachedMsID++)
				attachedMs(attachedMsID) = BS[bsID]->network->attachedMS[attachedMsID]; //attachedMS是基站连接的MS序列，连接代表此MS与在基站上RSRP最大

			arma::uvec *groupMetric; 
			groupMetric = new arma::uvec[numMaxLayer];
			arma::vec groupMetricValue; groupMetricValue.zeros(Sim.scheduling->numMaxLayer);
			...
			//cout << "0: " << groupMetric[1] << endl;
			arma::uvec temp = groupMetric[groupMetricValue.index_max()];//找出指标最大的组合数目，传几个用户
			int tempMS = 0;
			//cout << "1: " << arma::size(temp)(0) << endl;
			BS[bsID]->scheduling->scheduledMS.zeros(arma::size(temp)(0));
			for (int i = 0; i < arma::size(temp)(0); i++)
			{
				cout << "BS ID: " << bsID << endl;
				cout << "MS ID: " << temp(i) << endl;
				cout << "Buffer: " << MS[temp(i)]->scheduling->msBuffer << endl;
				if (MS[temp(i)]->scheduling->msBuffer != 0)//原来用的是network的msBuffer 并且temp存储的已经是MS的序号
				{
					BS[bsID]->scheduling->scheduledMS(tempMS) = temp(i);
					tempMS++;
					BS[bsID]->scheduling->numScheduledMS++;
				}
				else
				{
					BS[bsID]->scheduling->scheduledMS(tempMS) = -1;
				}
			}
...
void SchedulingBS::combinationUtil(arma::uvec arr, arma::uvec data, arma::uvec &groupMetric, double* value, int start, int end, int index, int r, int link)
{
	if (index == r)
	{
		...
		for (int j = 0; j < r; j++)
		{
			...
			if (link == 0)//0下1上
			{				
				spectralEfficiency = MS[data(j)]->scheduling->downlinkspectralEfficiency;//arr(j)改为data(j)是计算numMaxLayer中r个metricTemp指标最大的组合
				averagedThroghput = MS[data(j)]->scheduling->downlinkaveragedThroghput;
			}
			else
			{
				spectralEfficiency = MS[data(j)]->scheduling->uplinkspectralEfficiency;
				averagedThroghput = MS[data(j)]->scheduling->uplinkaveragedThroghput;
			}

			if (averagedThroghput == 0)
				metricTemp = spectralEfficiency / double(r);//平均值
			else
	...
	for (int i = start; i <= end && (end - i + 1) >= (r - index); i++) //r=numLayer + 1   index=0  end=BS[bsID]->channel->NumAssociatedMS-1  star=0
		//开始(end - i + 1) >= (r - index) = end-numLayer>=i
	{
		data(index) = arr(i);
		combinationUtil(arr, data, groupMetric, value, i + 1, end, index + 1, r, link); //index=r只有迭代到index + 1 = numLayer + 1，循环i=numLayer + 1到end
	}
}
