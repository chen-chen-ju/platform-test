...
int main()
{
	...
	Sim.Initialize("NReMBB_DenseUrban"); // NReMBB_UrbanMacroCell, NReMBB_DenseUrban, NReMBB_IndoorOffice, NReMBB_RuralMacroCell
...
void SystemSim::Demonstration() {
	...
	ofstream outFile;
	outFile.open("../../Simulator/Data/Output Data/Scheduled_embb_sinr.txt", ios::app);
	if (!outFile)
	{
		cout << "文件不能打开" << endl;
	}
	else
	{
		...
		{
			outFile << "MS index:  " << msID << "	SINR:  " << MS[msID]->scheduling->downlinkESINR << "	MCS:  " << MS[msID]->scheduling->MCS  << endl;
		}
		outFile << endl;
	}
	outFile.close();
