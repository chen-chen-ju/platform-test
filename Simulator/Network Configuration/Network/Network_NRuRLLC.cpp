...
void Network::Initialize(string fileName)
{
	...
	double A = sqrt(3) / 2;
	//posWraparound << 0 << 5 * A + 0.5i << 3 * A + 4i << -3 * A + 3.5i << -5 * A - 0.5i << -3 * A - 4i << 3 * A - 3.5i;//matlab画图显示第3和第6个有误，x坐标绝对值应该是2A
	posWraparound << 0 << 5 * A + 0.5i << 2 * A + 4i << -3 * A + 3.5i << -5 * A - 0.5i << -2 * A - 4i << 3 * A - 3.5i;
...
void Network::PlaceMacroBS()
{
	...
	numBS = bsID;
	numSite = numBS / numSector; //=numMacroBS
	setcolor(10, 0);
