...
//	arma::cx_mat x(57, 1);
//	arma::cx_mat y;
//	arma::field<arma::cx_mat> exp_vt(MAX_CLUSTER + 4); // dft 侩
//	arma::cx_mat h_k;
//	arma::cx_mat h_k1;
//	arma::cx_mat tmpp;
...
//				for (int m = 0; m < 20; m++) {
//					vt.real(0.0);
//					vt.imag(2.0 * PI*MS[src]->channel->VelocityBS(siIndex)(n, m) * double(Sim.TTI) / 1000.0); //[si] 哗绊
//					exp_vt[n](m, 0) = exp(vt);
//				}
...
void SpatialChannel::Delay(int src, int type) {
	...
	for (int n = 0; n < numCluster; n++) {
		...
		tauPrime(n, 0) = -rTau * lsp2 * log(arma::randu<double>()); //计算随机时延

		double test = log(10);
		if (temp > tauPrime(n, 0))
			temp = tauPrime(n, 0);//记录所有簇中的时延最小值
	}
...
void SpatialChannel::ClusterPower(int src, int dst, int type) {
	...
	for (int n = 0; n < numCluster; n++) {
		powerPrime[n] = exp(-delay(n,0) * (rTau - 1.0) / (rTau * lsp2)) * pow(10.0, -(perClusterShadowingStd * arma::randn()) / 10.0);//每径的功率
		if ((location == Outdoor) && (channelCondition == LOS)) // outdoor LOS牢 版快
			delaysLOS[n] = delay[n] / riceanKfactorDependentScalingConstantdB;//包含LOS径情况的时延
		sumPowerPrime = sumPowerPrime + powerPrime[n];//总功率
	}
	
	for (int n = 0; n < numCluster; n++) {
		clusterPower1[n] = powerPrime[n] / sumPowerPrime;//归一化
		
		if ((location == Outdoor) && (channelCondition == LOS))
			clusterPower[n] = 1.0 / (kR + 1.0) * clusterPower1[n] + (n == 0) * kR / (kR + 1.0);//包含LOS径，第一径添加LOS功率，其他NLOS径按公式处理
		else
	...
	for (int n = 0; n < numCluster; n++) {
		if (10.0*log10(clusterPower[n]) - 10.0*log10(maxClusterPower) < -25.0) //移除与最大功率相差25dB的簇
			clusterPower[n] = -100.0;

		if (clusterPower[n] != -100.0) {
			clusterPowerReduced[numRealCluster] = clusterPower[n];	//包含LOS径情况
			realClusterPowersForLOS[numRealCluster] = clusterPower1[n]; //归一化功率，无LOS径情况
...
void SpatialChannel::ArrivalAndDepartureAngle(int src, int dst, int site, int sector, int type) {
	...
	// Table 7.3-2 Scaling factors for AOA, AOD generation  我是7.5.2
	...
	// Table 7.3-3 Scaling factors for ZOA, ZOD generation  我是7.5.4
	arma::mat scalingFactorForZOAandZOD(2, 3);
	scalingFactorForZOAandZOD << 12 << 19 << 20 << arma::endr
		<< 1.104 << 1.184 << 1.178 << arma::endr;
	// Sector offset angle
	arma::mat alpha(3, 1);
	alpha << (PI / 180) * 30 << (PI / 180) * 150 << (PI / 180) *(-90) << arma::endr;//1/6, 5/6, 9/6 3个Sector
	...
	departureAngleGCS(0, 0) = AzimuthAngleOfGlobalCoordinateSystem(wraparoundposBS(site, 0), wraparoundposBS(site, 1), pos3D(0, 0), pos3D(0, 1));//arctan求角度（src,dst）,dst-src 水平θ角度
	...
	for (int m = 0; m < 11; m++) {
		if (numCluster == scalingFactorForAOAandAOD(0, m)) {
			cAOAAOD = scalingFactorForAOAandAOD(1, m); //AOA缩放因子
		}
	}
	...
	if ((location == Outdoor) && (channelCondition == LOS)) {
		cAOAAOD = cAOAAOD * (1.1035 - 0.028 *K - 0.002 * pow(K, 2.0) + 0.0001 * pow(K, 3.0)); //含LOS径，需要调整因子
	...
	for (int n = 0; n < numRealCluster; n++) {
		piAOAPrime(n, 0) = (2.0 * (sigmaASA / 1.4) * sqrt(-log(clusterPowerReduced(n, 0) / maxClusterPower))) / cAOAAOD;//第n簇的AOA
		...
		if (n == 0 || n == 1) {
			...
			for (int numRay = 0; numRay < 20; numRay++) {
				if (numRay < 10) {
					thetaRxRay[numRay] = thetaRayRxTemp0[numRay] - 1;//序号从0开始
		...
		xAOA[n] = (arma::randu() < 0.5) * 2.0 - 1.0;//7.5-11 Xn
		xAOD[n] = (arma::randu() < 0.5) * 2.0 - 1.0;
		xZOA[n] = (arma::randu() < 0.5) * 2.0 - 1.0;
		xZOD[n] = (arma::randu() < 0.5) * 2.0 - 1.0;
		yAOA[n] = arma::randn() * (largeScaleParameter(4, 0) / 7.0);//7.5-11 Yn
		...
		if ((location == Outdoor) && (channelCondition == LOS)) { //LOS情况
			clusterAOA = (xAOA[n] * piAOAPrime[n] + yAOA[n]) - (xAOA[0] * piAOAPrime[0] + yAOA[0] - (arrivalAngleGCS(0, 0)*180.0 / PI));//最后一部分单位是°。
		...
		for (int m = 0; m < 20; m++) {
			//先将角度映射到[0，360]°范围内
			RayAOD(m) = Modulo((clusterAOD + clusterASD * offsetAngle[m]), 360.0);//每个子径加上子径的偏移量
			...
			//再将角度映射到[0，180]°范围内  属于[180,380]°，映射为360°-θ
...
void SpatialChannel::ArrivalAndDepartureAntennaGain(int src, int dst, int site, int sector, int type) {
	...
	else
	{
		...
		location = UMS[src]->network->location;
		channelCondition = UMS[src]->channel->channelCondition;
		lsp2 = UMS[src]->channel->largeScaleParameter(2, 0);//DS
		UMS[src]->network->velocityHorizontal = (360 * arma::randu()*PI) / 180;
		UMS[src]->network->velocityVertical = (90 * PI) / 180;
		pos3D = UMS[src]->network->pos3D;
		wraparoundposBS = UMS[src]->network->wraparoundposBS;
	...
	// Antenna configuration zeta1, zeta2 极化方式
...
double SpatialChannel::AzimuthAngleOfGlobalCoordinateSystem(double srcX, double srcY, double dstX, double dstY) {
	...
	azimuthAngle = atan2(dstY-srcY,dstX-srcX);//弧度制
...
arma::mat SpatialChannel::LocalCoordinateSystemAntennaGainToGlobalCoordinateSystemAntennaGain(double alpha, double beta, double gamma, double theta, double pi, arma::mat antennaGain) { // LCS to GCS antenna gain
	...
	arma::mat psi(2,1);
	//公式7.1-16和7.1-17，第一个是cos,第二个是sin
...
double SpatialChannel::Modulo(double a, double b) {  //减去最多倍数的b
	if (b == 0)
	{
		return 0;
	}
...
//38.901 7.1-4 R a为方位角(bearing angle)，b为下倾角(downtilt angle)，c为倾斜角(slant angle)  R旋转变换
...
arma::cx_mat SpatialChannel::exp_F_urd(int src, arma::mat A, int numRxAntenna, int type) //A是到达角，AOA和ZOA
{
	arma::mat angleLCS, antennaOrientation;
	if (type == 0)
		antennaOrientation = MS[src]->channel->antennaOrientation;
	...
	for (int u = 0; u < numRxAntenna; u++) {
		if (Sim.channel->MsAntennaModel == CHANNEL::UniformLinearArray) {
			Rd = DistanceAngular(MS[src]->channel->antennaOrientation(0, 0) , antennaOrientation(1, 0), antennaOrientation(2, 0), 0.0, Sim.channel->WaveLengthOfElectomageneticWave * 0.5 * u, 0.0); //荐脚 救抛唱 芭府
