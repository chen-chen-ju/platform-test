...
void PerformanceURLLCMS::Measure()
{
	...
	{
		instantThroughput = UMS[id]->scheduling->downlinkspectralEfficiency * Sim.channel->NRuRLLC.bandwidth / 10 * 50 * (12 * 11 - 12 - 16) * 1000;//吞吐量是按照使用全部RB计算的？
		//num_RB= 
		//instantThroughput = UMS[id]->scheduling->GetTBsize(UMS[id]->scheduling->downlinkspectralEfficiency , (int)1); //使用几个PRB
		...
		*/
		...
		cout << "id:  " << id << "   delete-Buffer:  " << instantThroughput << "   Buffer:  " << UMS[id]->scheduling->msBuffer << endl;
		cout << "Efficiency:  " << UMS[id]->scheduling->downlinkspectralEfficiency << endl;
		cout << "SINR:  " << temp << "  MCS:  " << UMS[id]->scheduling->MCS << endl;
	}
	else
	{
		instantThroughput = 0;
