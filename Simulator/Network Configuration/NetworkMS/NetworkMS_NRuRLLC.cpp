...
void NetworkMS::BufferUpdate()
{
	if ((Sim.TTI == arrivalTime))
	{
		...
		arrivalTime = Sim.TTI + interArrivalTime(Sim.TTI);
	}
...
void NetworkMS::PlaceRandomHexagonal()
{
	...
	if (Sim.network->NetworkModel == NETWORK::UrbanMacroCell) {
		...
		while (minDistanceCheck == 0)
		{
			...
			p = exp(1i * 2.0 * PI * (1 / 2 + floor(arma::randu() * 3) / 3)) * p; //得到六边形的均匀分布
			p = 1.0 / 2.0 + 1i / (2.0 * sqrt(3)) + p; //将原点放置在六边形的右下角
			p = exp(1i * 2.0 * PI * (double)(attachedBS - 1 % 3) / 3.0) * p;//旋转构成三小区（3个紧密六边形）结构
			pos = BS[this->attachedBS]->network->pos + p * 2.0 / 3.0 * A * Sim.network->UrbanMacroCell.interSiteDistance;
			//第一项为所在小区的绝对位置，第二项为移动台与所在小区的相对距离,系数是放大六边形区域，使得同频小区中心距离为UrbanMacroCell.interSiteDistance（使用的是i=j=1的频率复用）
...
void NetworkMS::WrapAround()
{
	...
	{
		for (int j = 0; j < Sim.network->numWraparound; j++) //为每个site设置一个wraparound
		{
			vector = abs(MS[id]->network->pos - (Sim.network->posWraparound[j] + BS[Sim.network->numSector * i]->network->pos)); //求模值
			temp(j) = vector;
		}
		arma::uword index = temp.index_min();
		wraparound(i) = Sim.network->posWraparound[index] + BS[Sim.network->numSector * i]->network->pos; //最近的交换点相对位置与基站绝对位置之和
	}
	x = arma::real(arma::trans(wraparound));//trans共轭转置
	y = -arma::imag(arma::trans(wraparound));
	//逐个元素取虚、实部
	this->wraparoundposBS.submat(arma::span(0, Sim.network->numSite - 1), arma::span(0, 0)) = x;//submat矩阵的子矩阵元素，参数（行范围，列范围）
...
arma::mat NetworkMS::LargeScaleParameter(int channelModel, int channelCondition, int msID, int bsID) {
	...
	largeScaleParameter.zeros(7, 1);
	arma::mat correlationMatrixCholeskyDecomposed(7,7);//Cholesky 分解是把一个对称正定的矩阵表示成一个下三角矩阵L和其转置的乘积的分解
	...
	//文档中表示的是 pow(10.0, spatialChannel.UrbanMacroCellLOS.sigmaShadowing*LargeScaleParameter(0)/10);
